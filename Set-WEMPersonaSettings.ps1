...
function Set-WEMPersonaSettings {
    ...
    process {
        ...
        foreach($key in $keys) {
            ...
            $SQLQuery = "UPDATE VUEMPersonaSettings SET Value = '$($value)', RevisionId = RevisionId + 1 WHERE Name = '$($key)' AND IdSite = $($IdSite)"
            ...
            New-ChangesLogEntry -Connection $Connection -IdSite $IdSite -IdElement $IdObject -ChangeType "Update" -ObjectName $key -ObjectType "Policies and Profiles\VMware Persona Settings" -NewValue $value -ChangeDescription $null -Reserved01 $null 
        }
    }
}
...
function Reset-WEMPersonaSettings {
    ...
    process {
        ...
        Set-WEMPersonaSettings -Connection $Connection -IdSite $IdSite -Parameters $parameterObject
    }
}
