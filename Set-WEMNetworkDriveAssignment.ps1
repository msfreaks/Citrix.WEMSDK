<#
    .Synopsis
    Updates a Network Drive Assignment object in the WEM Database.
    ...
    Author: Arjan Mensch
#>
function Set-WEMNetworkDriveAssignment {
    [CmdletBinding()]
    param (
    ...
    process {
        ...
        $origObject = Get-WEMAssignment -Connection $Connection -IdAssignment $IdAssignment -AssignmentType "Network Drive"

        # only continue if the object was found
        if (-not $origObject) { 
            Write-Warning "No Network Drive assignment found for Id $($IdAssignment)"
            Break
        }
        ...
        if ($checkADObject -or $checkRule) {
            $SQLQuery = "SELECT COUNT(*) AS ObjectCount FROM VUEMAssignedNetDrives WHERE IdSite = $($origObject.IdSite) AND IdNetDrive = $($origObject.IdAssignedObject)"
            ...
            if ($result.Tables.Rows.ObjectCount) {
                # name must be unique
                Write-Error "There's already another Network Drive assignment matching those Ids in the Configuration"
                Break
            }
        ...
        if ($checkADObject -or $checkRule -or $checkProperties) {
            $SQLQuery = "UPDATE VUEMAssignedNetDrives SET "
        ...
        if($updateFields) { 
            ...
            $SQLQuery += "RevisionId = $($origObject.Version + 1) WHERE IdAssignedNetDrive = $($IdAssignment)"
            $null = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # grab the updated assignment
            $SQLQuery = "SELECT * FROM VUEMAssignedNetDrives WHERE IdAssignedNetDrive = $($IdAssignment)"
            $result = Invoke-SQL -Connection $Connection -Query $SQLQuery
    
            $Assignment = Get-WEMAssignment -Connection $Connection -IdAssignment $IdAssignment -AssignmentType "Network Drive"

            # Updating the ChangeLog
            $IdObject = $result.Tables.Rows.IdAssignedNetDrive
            New-ChangesLogEntry -Connection $Connection -IdSite $origObject.IdSite -IdElement $IdObject -ChangeType "Assign" -ObjectName $Assignment.ToString() -ObjectType "Assignments\Network Drive" -NewValue "N/A" -ChangeDescription $null -Reserved01 $null
        } else {
            Write-Warning "No parameters to update were provided"
        }
    }
}
New-Alias -Name Set-WEMNetDriveAssignment -Value Set-WEMNetworkDriveAssignment
