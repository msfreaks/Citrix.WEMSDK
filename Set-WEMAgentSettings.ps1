<#
    .Synopsis
    Updates a WEM Agent Settings object in the WEM Database.
...
function Set-WEMAgentSettings {
    ...
    process {
        ...
        $Parameters.Keys | ForEach-Object {
            if ($configurationSettings[$script:databaseSchema].AgentSettingsValues -match "'$($_)'") {
        ...
        $keys = $MyInvocation.BoundParameters.Keys | Where-Object { $configurationSettings[$script:databaseSchema].AgentSettingsValues -match "'$($_)'" }

        foreach($key in $keys) {
            # build query for each valid parameter
            $value = (Get-Variable -Name $key).Value
            $SQLQuery = "UPDATE VUEMAgentSettings SET Value = '$($value)', RevisionId = RevisionId + 1 WHERE Name = '$($key)' AND IdSite = $($IdSite)"

            # execute the query
            $null = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # grab the updated object
            $SQLQuery = "SELECT * FROM VUEMAgentSettings WHERE IdSite = $($IdSite) AND Name = '$($key)'"
...
<#
    .Synopsis
    Resets a WEM Agent Settings object in the WEM Database.
    ...
    Version: 0.9.0
#>
function Reset-WEMAgentSettings {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$True,ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [int]$IdSite,
    ...
    process {
        ...
        $parameterObject = @{}
        foreach($key in $configurationSettings[$script:databaseSchema].AgentSettingsValues) {
            $fields = $key.Split(",")
        ...
        Set-WEMAgentSettings -Connection $Connection -IdSite $IdSite -Parameters $parameterObject
    }
}
