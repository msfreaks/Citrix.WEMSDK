<#
    .Synopsis
    Updates a WEM System Monitoring Settings object in the WEM Database.
    ...
    Version: 0.9.0
#>
function Set-WEMSystemMonitoringSettings  {
    [CmdletBinding()]
    param(
    ...
    process {
        ...
        $Parameters.Keys | ForEach-Object {
            if ($configurationSettings[$script:databaseSchema].SystemMonitoringValues -match "'$($_)'") {
        ...
        $keys = $MyInvocation.BoundParameters.Keys | Where-Object { $configurationSettings[$script:databaseSchema].SystemMonitoringValues -match "'$($_)'" }

        foreach($key in $keys) {
            # build query for each valid parameter
            $value = (Get-Variable -Name $key).Value
            $SQLQuery = "UPDATE VUEMSystemMonitoringSettings SET Value = '$($value)', RevisionId = RevisionId + 1 WHERE Name = '$($key)' AND IdSite = $($IdSite)"

            # execute the query
            $null = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # grab the updated object
            $SQLQuery = "SELECT * FROM VUEMSystemMonitoringSettings WHERE IdSite = $($IdSite) AND Name = '$($key)'"
            $result = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # Updating the ChangeLog
            $IdObject = $result.Tables.Rows.IdItem
            New-ChangesLogEntry -Connection $Connection -IdSite $IdSite -IdElement $IdObject -ChangeType "Update" -ObjectName $key -ObjectType "Monitoring\System Monitoring Setting" -NewValue $value -ChangeDescription $null -Reserved01 $null 
        }
    }
}
...
<#
    .Synopsis
    Resets a WEM System Monitoring Settings object in the WEM Database.
    ...
    Version: 0.9.0
#>
function Reset-WEMSystemMonitoringSettings {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$True,ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [int]$IdSite,
    ...
    process {
        ...
        $parameterObject = @{}
        foreach($key in $configurationSettings[$script:databaseSchema].SystemMonitoringValues) {
        ...
        Set-WEMSystemMonitoringSettings -Connection $Connection -IdSite $IdSite -Parameters $parameterObject
    }
}
