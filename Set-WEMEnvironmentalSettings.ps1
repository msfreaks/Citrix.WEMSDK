...
function Set-WEMEnvironmentalSettings {
    ...
    process {
        ...
        $Parameters.Keys | ForEach-Object {
            if ($configurationSettings[$script:databaseSchema].EnvironmentalValues -match "'$($_)'") {
        ...
        $keys = $MyInvocation.BoundParameters.Keys | Where-Object { $configurationSettings[$script:databaseSchema].EnvironmentalValues -match "'$($_)'" }

        foreach($key in $keys) {
            # build query for each valid parameter
            $value = (Get-Variable -Name $key).Value
            $SQLQuery = "UPDATE VUEMEnvironmentalSettings SET Value = '$($value)', RevisionId = RevisionId + 1 WHERE Name = '$($key)' AND IdSite = $($IdSite)"

            # execute the query
            $null = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # grab the updated object
            $SQLQuery = "SELECT * FROM VUEMEnvironmentalSettings WHERE IdSite = $($IdSite) AND Name = '$($key)'"
            $result = Invoke-SQL -Connection $Connection -Query $SQLQuery

            # Updating the ChangeLog
            $IdObject = $result.Tables.Rows.IdItem
            New-ChangesLogEntry -Connection $Connection -IdSite $IdSite -IdElement $IdObject -ChangeType "Update" -ObjectName $key -ObjectType "Policies and Profiles\Environmental Settings" -NewValue $value -ChangeDescription $null -Reserved01 $null 
        }
    }
}
...
function Reset-WEMEnvironmentalSettings {
    ...
    process {
        ...
        $parameterObject = @{}
        foreach($key in $configurationSettings[$script:databaseSchema].EnvironmentalValues) {
        ...
        Set-WEMEnvironmentalSettings -Connection $Connection -IdSite $IdSite -Parameters $parameterObject
    }
}
